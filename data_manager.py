...
def create_dataframe(teams_data):
    ...
    # Iterate through the team and player data to fill in DataFrame rows
    ...
    if 'Team' not in df.columns or 'Player Name' not in df.columns:
        raise KeyError("Columns 'Team' and 'Player Name' are missing in the DataFrame")
...
def clean_stat_value(raw_text, index, is_percentage=False):
    ...
    - is_percentage (bool): If True, removes % and converts to decimal format.
    
    Returns:
    - float: The cleaned and converted value.
    """
    # Extract the specific word based on the index and remove parentheses
    value_str = re.sub(r'\(.*?\)', '', raw_text.split()[index]).strip()
